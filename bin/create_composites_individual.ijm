...
for (k = 0; k< runs.length;  k++)	{
	...
	if(startsWith(runs[k], "run")) {
		...
		for (h = slideDirList.length -1 ; h >= 0; h--) {
			...
			for (j = 0; j < roiList.length; j++) {
				...
				for (m = 0; m < stacks.length; m++)	{
					...
					// panactin overlaps with DNA and tumour intensities
					...
					run("Max (3D)");
					saveAs('Tiff',  outDir + "mask" + tma + "-" + roi + ".tiff");
                    setAutoThreshold("Triangle dark");
                    run("Convert to Mask");
					...
					run("Duplicate...", "title=DNA_mask");
                    setAutoThreshold("Default dark");
                    run("Convert to Mask");
                    run("Invert");
                    run("32-bit");
                    run("Calculator Plus", "i1=DNA_mask i2=DNA_mask operation=[Divide: i2 = (i1/i2) x k1 + k2] k1=1 k2=0");
                    run("Calculator Plus", "i1=SUM_Stroma i2=DNA_mask operation=[Multiply: i2 = (i1*i2) x k1 + k2] k1=1 k2=0 create");
                    selectWindow("Result");
