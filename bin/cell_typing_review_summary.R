...
#features=c("area", "meanIntensity", "probability", "meanUpperIntensity",
#           "maxPropThreshold", "maxPropZero")
features=c('area', 'meanIntensity', 'probability')
leukocytes=c("T cells", "Leukocytes", "CD4 T cells", "CD8 T cells")
...
if(args$panel == "p1" & args$cohort != 'peace') {
  ...
  excludedCell=c("Mesenchymal cells")
  dependentCell=c()
  mostLikelyCells=c('Leukocytes', 'Leukocytes')
} else if(args$panel == "p2" & args$cohort!='peace') {
  markerSets=c("mcsa", "mcsamy")
  excludedCell = c("Myeloid cells")
  mostLikelyCells=c("Leukocytes", "Leukocytes")
  dependentCell=c("Monocytes", 'Macrophages')
  # markerSets=c("mcsa", "mcsanov")
  # excludedCell = c("Endothelial cells")
  # mostLikelyCells=c("Leukocytes", "Epithelial cells")
  # Released
  # dependentCell=c()
  # mostLikelyCell="Leukocytes"
  # markerSets=c("mcsa", "mcsanov")
  # excludedCell = c("Monocytes")
}  else if(args$panel == "p2" & args$cohort=='peace') {
  markerSets=c("mcsa", "mcsapeace")
  excludedCell = c("Leukocytes", "None")
  # Check on the image: P2_TMA017_20201217_roi_5
  mostLikelyCells=c("Leukocytes", "Myeloid cells") # Endothelial cells
  dependentCell=c("")
}
...
for(markers in markerSets) {
  for(method in methods) {
	with(args, print(f(analysisPath)))
    ...
    # tmp$cellType =  gsub(" T cells", "", tmp$cellType)
	if(method == "csm") {
		table(tmp$positive)
    ...
    if(markers==markerSets[2] & args$cohort=='tx100') {
      # e.g. Myeloid cells vs Leukocytes in mcsamy
...
table(paste(mrg$method, mrg$positive))
# Exclude unassigned, esp from csm, so that there are not false negatives
...
for(feature in grep("Intensity", features, value = T))
...
#recast$cellType_cellassign_mcsamy[recast$cellID %in% grep('P2_TMA001_L_20190508-roi_11', negativeExcluded, value = T)] %>% table
...
# controlSet=mrg[controlCellIDs, ]
# controlSet$control=sapply(controlSet$control == "positive", sum)
# print(table(controlSet$control, paste(controlSet$region, controlSet$cellType)))
# bestModel=step_backward_lme4_model(features, data=dfFlt)
# best2Model=step_forward_lme4_model(features, data=controlSet)
# save(bestModel, best2Model, file=gsub(".RData",  ".best.RData", f("{modelOut}")))

# g< - ggplot(mrg[posExcludedIndices | negExcludedIndices,], aes(control, meanIntensity))
# g + geom_boxplot()
# Check that the markers have been correctly assigned
# model <- nlme::lme(control ~ probability, random=~1|cellType, data=dfFlt) #, family=binomial
# model <- lme4::glmer(as.formula(f("control ~ ", paste0(features, collapse="+"),
...
print(plot)
dev.off()
# browseURL(f("{out}/Model.{analysisID}.pdf"))
# P1_TMA005_R_20190619-roi_21
...
plot=g + geom_bar(stat="identity") + theme_classic(base_size=18) +
  ...
  theme(axis.line=element_blank(),
        axis.text= element_blank(), #element_text(size=10, color="black"),
...
for(feature in features) {
  ...
  dfMrg=droplevels(mrg[mrg$method == "cellassign" & mrg$markers == markerSets[2] |
                         mrg$cellType %in% excludedCell & mrg$markers== markerSets[1], ])
...
for(imagename in rev(imagenames)) {
  ...
  for(analysis in grep("cellType_", colnames(recast), value = T)) {
    ...
    for(region in c("negative", "positive")) { #, "Background", names(table(imageDf$region)))
