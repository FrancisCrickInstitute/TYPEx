...
run_method <- function(inData, method, pars, runID, wDir, regFile, nfDir,
  ...
  if(! file.exists(inMatFile))  {
    ...
    if(file.exists(clFile)) {
      load(clFile)
      rowsKeep=setdiff(rowsKeep, which(is.na(clusters)))
...
run_umap<-function(inMat, p, colors)  {
  ...
  if(file.exists(outFile)) {
    ...
    out=umap::umap(setDF(inMat), random_state=555)
    save(out, colors, p, file = outFile)
  }
  umapPdf=paste0(p$run_id, ".umap.pdf")
  pdf(file=umapPdf, width=7, height = 7)
  plotUmap(x=out, labels=colors, colors=cellTypeColors, legend=F)
  dev.off()
}

determine_threshold<-function(assigned, clusterSize, runID, confidence='low', breakStep=0.001, greater = F) {
...
#	print(pos)
#	print(lapply(pos, function(x) length(x) = 0 ))
#	print(all(unlist(lapply(pos, function(x) length(x))) == 0 ))
     if(all(unlist(lapply(pos, function(x) length(x))) == 0 )) {
		return(sapply(combos, function(x) {
			return(0)
		}))
	}
	past=apply(pos, 1, function(x) {
		print(x)
  ...
  if(all(mat == 0)) {
	return(sapply(setdiff(colnames(mark), c('cluster', 'sizes')), ''))
  }	
  mat$D=breaks
  
  pdfOut=f('{runID}.positivity_calling_kstest.pdf')
  pdf(pdfOut, height=3, width = 4)
  g<-ggplot(mat, aes(D)) 
  plot= g +
    geom_path(aes(y=`CD3_CD4_CD8a`, color="CD3_CD4_CD8a")) +
    scale_color_manual(values=c('CD3_CD4_CD8a'='green',
                                'CD3_CD8a'="blue",
                                "CD3_CD4"='black')) +
    geom_path(aes(y=`CD4_CD8a`), color='green', linetype='dotted') +
    geom_path(aes(y=`CD3_CD8a`, color='CD3_CD8a')) +
    geom_path(aes(y=`CD3_CD4`, color='CD3_CD4')) +
    geom_path(aes(y=`CD4`), color='black', linetype='dotted')  +
    geom_path(aes(y=`CD8a`), color='blue', linetype='dotted')  +
    geom_path(aes(y=`CD3`), color='red') +
    theme_classic() + theme(legend.position = 'top') +
    scale_y_log10()
  
  tcell_separation=mat$CD3_CD4 + mat$CD3_CD8a - mat$CD3_CD4_CD8a - mat$`CD4_CD8a`
  tcell_pct=sapply(1:nrow(mat), function(x)
    sum(mat$CD3_CD4_CD8a[x] + mat$`CD4_CD8a`[x])/sum(mat$CD3_CD4[x] + mat$CD3_CD8a[x] + mat$CD3_CD4_CD8a[x] + mat$`CD4_CD8a`[x]))
  tcell_pct[tcell_pct == 0] =NA
  ...
  print(plot + geom_vline(xintercept=mat$D[Dsel], linetype='dotted'))
  dev.off()
  # browseURL(pdfOut)
