...
assign_celltype<-function(names, markers, majorTypes=NULL, region=NULL, major=F) {
  ...
  celltypes=sapply(clusters, function(name) {
    ...
    if(!length(cellMarkers)) {
	  ...
	  if(grepl('Epithelial cells', majorType) & region != 'none' & region != name & region > 0) return("Epithelial cells Region")
    ...
    if(sum(assigned==maxas & maxas > 0, na.rm = T) > 1) {
      if(! majorType %in% c("none")) return(majorType)
      if(region != 'none' & region != name & region > 0) return("Epithelial cells Region")
      return("Ambiguous")
    }
    ...
    if(length(intersect) > 1 & all(intersect == 0) & majorType != 'none') {
      if(! majorType %in% c("none") & !major) return(majorType)
    ...
    if(major & sum(assigned==maxas, na.rm = T) > 1 & majorType == "none" & region != 'none' & region != name & region > 0) return("Epithelial cells Region")
